	...
	for char in b:
		match char:
			case 'b':
				...
				system('sudo apt -qqq update && sudo apt -qqq install brave-browser -y')
			case 'c':
			...
			case 'e':
				...
				system('sudo apt -qqq install microsoft-edge-stable -y')
			case 'f':
				system('sudo apt -qqq install firefox -y')
...
def apps():
	...
	for char in a:
		match char:
			case 's':
				system('sudo apt -qqq install spotify-client -y')
			case 'd':
				system('sudo snap install discord')
			case 'c':
				system('sudo apt -qqq install code -y')
			case 'v':
				system('sudo apt -qqq install vlc -y')
